...
public class ModTreeGeneration
{
    public static void generateTrees(final BiomeLoadingEvent event)
    {
        ...
        {
            List<Holder<PlacedFeature>> hillPlace =
                    event.getGeneration().getFeatures(GenerationStep.Decoration.VEGETAL_DECORATION);

            hillPlace.add(ModPlacedFeatures.IRONWOOD_PLACED);
        }

        if(types.contains(BiomeDictionary.Type.SAVANNA))
        {
            List<Holder<PlacedFeature>> savannaPlace =
                    event.getGeneration().getFeatures(GenerationStep.Decoration.VEGETAL_DECORATION);

            savannaPlace.add(ModPlacedFeatures.OSAGE_PLACED);
        }
    }
}
